        ...
        if (jsonResponse['status'] == '200' && jsonResponse['data'] != null) {
          ...
          try {
            final imgUri = Uri.parse(
              'https://app.beatflirtevent.com/App/user/signle_user_profile_image',
            );
            ...
            if (imgResponse.statusCode == 200) {
              final decodedImg = jsonDecode(imgResponse.body);
              if (decodedImg is Map && decodedImg['status']?.toString() == '200') {
                final imgList = decodedImg['data'];
                if (imgList is List && imgList.isNotEmpty) {
                  Map<String, dynamic>? selectedImg;
                  for (final item in imgList) {
                    if (item is Map && item['set_profile']?.toString() == '1') {
                      selectedImg = Map<String, dynamic>.from(item);
                      break;
                    }
                  }
                  selectedImg ??= Map<String, dynamic>.from(imgList.first as Map);
                  final rawPath = selectedImg['profile_image']?.toString() ?? '';
...
class ProfileCard extends StatelessWidget {
  ...
  const ProfileCard({super.key, required this.data, this.profileImageUrl});
